...
require "../../componentes/com_sistema/class.componentes.php";
...
echo "<script type=\"text/javascript\"> setTimeout(\"window.top.location='class.upload.php?lagc=".$_GET['lagc']."&selec=".$nombrefinal.".".$tipofin."'\", 1000) </script>";
        ...
        <?=Img::listar_ficheros($tipos, "../../../componentes/com_imagenes/imagenes/", 4);
	}
				...
				if ($scanarray[$i] != "." && $scanarray[$i] != ".."){
					...
					if (is_file($carpeta . "/" . $scanarray[$i])){
						...
						if (in_array ($thepath['extension'], $tipos)){
							...
							echo "<a href=\"#\" onClick=\"insertImg('$scanarray[$i]')\"><img src=\"".$carpeta."".$scanarray[$i]."\" width=\"".Img::__TamaniosImg($carpeta."/".$scanarray[$i])."\" border=\"0\"></a>";
							echo "</div>\n";
							$contador++;
							echo "</td>\n";
						}
...
function insertImg($valor){
	...
	document.selecimg.imgpreviw.src = '../../../componentes/com_imagenes/imagenes/'+$valor;
}
</script>
</head>
<body onLoad="centrar()">
