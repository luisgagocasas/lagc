...
<form name="frmcont" method="post" action="">
  <center>
    <table border="0" cellpadding="5">
      <tr>
        ...
        <h4>Titulo</h4>
          <input name="titulo" type="text" class="inp-form" maxlength="225" size="36" value="<?=$articu['b_titulo']; ?>">
          </td>
          <td style="vertical-align:top;">
         <h4>Usuario</h4>
         <?=LGlobal::Url_Usuario($articu['b_autor'], true, false, "", ""); ?>
          </td>
          <td style="vertical-align:top;"><?php if(!empty($articu['b_fechamodi'])){ echo "<h4>Ultima vez modificado</h4> ".date("Y/m/d", $articu['b_fechamodi']); } ?></td>
          </tr>
          </table>
          </td>
      </tr>
      <tr>
        <td><h4>Resumen</h4>
          <textarea id="form-textarea" name="resumen" onkeypress="return limita(event, 255, 'form-textarea');" onkeyup="actualizaInfo(255, 'form-textarea', 'info')" class="mceNoEditor" cols="40" rows="5"><?=$articu['b_resumen']; ?></textarea>
          <br />
          <div id="info" style="color:#999;">M&aacute;ximo 255 caracteres</div></td>
        <td><h4>Imagen</h4>
          <div style="margin:0px 0px 0px 0px;"></div>
          <div class="fondoimg" style="width:160px; height:100px; cursor:pointer;" onclick="abrirventana('1');"> <img src="componentes/com_imagenes/thumbnails/<?=$articu['b_img']; ?>" name="img1" border="0" /> </div>
          <input type="text" name="img1" value="<?=$articu['b_img']; ?>" onfocus="document.images.img1.src='componentes/com_imagenes/thumbnails/'+this.value" class="borderinputimg"></td>
      </tr>
      <tr>
        <td><h4>Palabras Claves</h4>
          <input name="palabras" type="text" size="65" class="inp-form" value="<?=$articu['b_palabras']; ?>" /><br />
        ...
        <td><h4>Categorias</h4>
          ...
          while($cat = mysql_fetch_array($respcat)) {
              if($articu['b_categoria']==$cat['cat_id']) {
      ...
      <tr>
        <td colspan="2" align="left"><h4>Descripci&oacute;n Completa</h4>
          <label><textarea name="contenido" cols="60" rows="30"><?=$articu['b_contenido']; ?></textarea></label>
          </td>
      </tr>
    </table>
  </center><br />
  ...
  <blockquote>
    <ul>
      <li>&raquo; Cual quier consulta al (Webmaster) "<a href="/usuarios/1-vikingo">Click Aqui</a>" o a luisgago@lagc-peru.com</li>
    </ul>
  </blockquote>
<br />
<br />
